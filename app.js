...
const CATEGORIES = [
  { id:"philosophy", name:"교육철학", weight:20, items:[
    ...
    ["아이가 싫어하는 활동일 때 자율성을 존중한다", "Q", "아이가 활동을 거부했던 최근 사례에서 선생님은 어떻게 도와주셨나요?"],
    ["원장의 교육철학이 실제 생활지도와 우리 가족에게 잘 맞는다", "Q", "원장님이 가장 중요하게 생각하는 유아교육 원칙은 무엇이며, 아이가 지시를 따르지 않을 때 그 원칙을 어떻게 적용하시나요?"],
  ]},
  { id:"teacher", name:"선생님", weight:25, star:true, items:[
    ...
    ["아이의 감정을 먼저 읽고 안정시킨다", "O", "낯설어하거나 우는 아이에게 눈높이를 맞추고 감정을 말로 확인해주는지 관찰"],
    ["아이 간 갈등을 벌보다 대화와 관계 회복으로 중재한다", "Q", "아이들끼리 다툰 최근 사례에서 어떤 순서로 상황을 확인하고 중재하셨나요?"],
    ["부모와의 소통이 구체적이고 방어적이지 않다", "Q", "아이의 걱정되는 변화를 발견하거나 부모가 우려를 제기하면 누가, 언제, 어떤 절차로 확인하고 결과를 알려주시나요?"],
  ]},
  ...
  { id:"safety", name:"안전·적응", weight:20, items:[
    ...
    ["담임 부재·교체 시에도 돌봄이 안정적으로 이어진다", "Q", "담임 선생님이 갑자기 쉬거나 중간에 교체되면 누가 맡고 아이의 생활정보를 어떻게 인계하나요?"],
  ]},
  { id:"practical", name:"현실 조건", weight:15, items:[
    ...
    ["등·하원과 방과후 운영시간이 우리 생활과 맞다", "Q", "실제로 맡길 수 있는 가장 이른 시간과 가장 늦은 시간은 언제이며, 늦은 시간에는 누가 아이들을 돌보나요?"],
    ["방학 중 돌봄·급식 운영이 우리 생활과 맞다", "Q", "방학 중 미운영 날짜는 며칠이며, 운영일의 돌봄 시간·급식·추가비용은 어떻게 되나요?"],
  ]},
];
...
const DISQ = [
  ...
  ["CCTV 설치 위치와 열람 절차가 명확하다", "CCTV는 주요 공간 어디에 설치되어 있고, 필요할 때 열람을 신청하는 절차는 어떻게 되나요?"],
  ["시설 안전점검·안전교육·보험 이력이 확인된다", "유치원알리미에서 최근 안전점검, 안전교육 실시, 공제회·보험 가입 현황 확인"],
];
const TYPE_META = {
  Q: { label:"질문", color:"#2F7D5B", bg:"#EAF4EE" },
  O: { label:"관찰", color:"#B07A00", bg:"#FFF6DC" },
  R: { label:"사전확인", color:"#5A6ACF", bg:"#ECEEFB" },
};
const INFO_GROUPS = [
...
function renderDetail(kg){
  ...
  if(detailTab==="score"){
    ...
    body += `<p class="q-intro">상담 때 그대로 물어보거나 관찰하시면 돼요. <b style="color:var(--green);">질문</b>은 원에 묻는 것, <b style="color:#B07A00;">관찰</b>은 눈으로 볼 것, <b style="color:#5A6ACF;">사전확인</b>은 유치원알리미·공시자료로 미리 확인할 것이에요.</p>`;
